...
long get_sort_time(u_int32_t *arr, size_t arr_size, void (*sorting)(u_int32_t *array, size_t size));
u_int32_t  *create_arr(size_t size);
...
long test(size_t arr_size) {
    u_int32_t  *arr  = create_arr(arr_size);

    long time = get_sort_time(arr, arr_size, lsd_sort);
...
u_int32_t *create_arr(size_t size) {
    u_int32_t *arr = (u_int32_t*) calloc(size, sizeof(u_int32_t));
